...
float getPoids(){
    ...
    for(i=0;i<100;i++){
      HAL_ADC_Start(&hadc1);// lance L'ADC
      HAL_ADC_PollForConversion(&hadc1,100);
      adcValeur= HAL_ADC_GetValue(&hadc1); //la foncton HAL_ADC_PollForConversion pour récuperer la valeur de l'ADC avec l'adresse de l'ADC en paramètre
